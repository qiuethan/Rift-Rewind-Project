...
class AuthRepositorySupabase(AuthRepository):
    ...
    async def create_user(self, email: str, password: str, user_data: dict) -> Optional[AuthResponse]:
        """Create a new user"""
        if self.client:
            auth_response = await self.client.auth_sign_up(email, password)
            
            user_id = auth_response.user.id
            token = auth_response.session.access_token
            
            await self.client.table(DatabaseTable.USERS).insert({
                'id': user_id,
                'email': email,
                **user_data
            }).execute()
    ...
    async def get_user_by_email(self, email: str) -> Optional[dict]:
        ...
        if self.client:
            response = await self.client.table(DatabaseTable.USERS).select('*').eq('email', email).limit(1).execute()
            if response.data:
    ...
    async def get_user_by_id(self, user_id: str) -> Optional[dict]:
        ...
        if self.client:
            response = await self.client.table(DatabaseTable.USERS).select('*').eq('id', user_id).limit(1).execute()
            if response.data:
                return response.data[0]
        return None
    ...
    async def verify_password(self, email: str, password: str) -> bool:
        ...
        if self.client:
            auth_response = await self.client.auth_sign_in(email, password)
    ...
    async def update_user(self, user_id: str, user_data: dict) -> Optional[dict]:
        ...
        if self.client:
            response = await self.client.table(DatabaseTable.USERS).update(user_data).eq('id', user_id).execute()
            if response.data:
                return response.data[0]
        return None
